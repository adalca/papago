function params = mstepGPULatentSubspace(wg, data)
% assumes all arrays are GPU arrays

    % data
    Y = data.gpuY;
    if isfield(data, 'gpuW')
        obsMask = data.gpuW;
    ...
    gammank = gpuArray(wg.expect.gammank);
    
    wg.params.W = gpuArray(single(wg.params.W));
    wg.params.sigmasq = gpuArray(single(wg.params.sigmasq));
    ...
    mu = zeros(K, dHigh, 'single', 'gpuArray');
    newW = zeros(dHigh, dLow, K, 'single', 'gpuArray');
    newsigmasq = zeros(1, K, 'single', 'gpuArray');
    ...
    for k = 1:K
        ...
        gEffLog = gammank(:, k) >= 1e-4;
        gEffIdx = find(gEffLog);
        gEff = gammank(gEffIdx, k);
        nEff = numel(gEffIdx);
        ...
        Shat = zeros(dLow, dLow, nEff, 'single', 'gpuArray');
        Xhat = zeros(N, dLow, 'single', 'gpuArray');
        
        edl = eye(dLow, 'gpuArray');
        ...
        % update mu
        muDenom = zeros(1, dHigh, 'single', 'gpuArray');
        for ei = 1:nEff
            i = gEffIdx(ei);
            gnk = gEff(ei);
